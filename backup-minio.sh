...
# Fitur:
#   1. Deteksi otomatis kontainer MinIO, network Docker, port, dan kredensial.
...
if [ -z "$MINIO_CONTAINER" ]; then
    ...
    echo "  docker compose -f docker-compose.base.yml up -d minio"
    echo "  (atau: ./rsch infra up minio)"
    exit 1
fi
...
if [ -z "$MINIO_USER" ] || [ -z "$MINIO_PASSWORD" ]; then
    # Fallback jika printenv kosong: cek file .env
    MINIO_USER="admin"
    MINIO_PASSWORD="password"
fi
log_info "Kredensial MinIO root terverifikasi: user='${MINIO_USER}'"

# Deteksi port internal MinIO (9090 di master vs 9000 di restructure)
INTERNAL_PORT="9090"
if docker exec "$MINIO_CONTAINER" curl -s http://localhost:9090/minio/health/ready >/dev/null 2>&1; then
    INTERNAL_PORT="9090"
elif docker exec "$MINIO_CONTAINER" curl -s http://localhost:9000/minio/health/ready >/dev/null 2>&1; then
    INTERNAL_PORT="9000"
fi
