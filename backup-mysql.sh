...
# Fitur:
#   1. Akses kontainer MySQL via Docker Compose (service: db).
...
# Pindai file .env di folder env/ untuk menemukan database aplikasi tambahan
for env_file in "${SCRIPT_DIR}"/env/.env.* "${SCRIPT_DIR}"/.env*; do
    [ -f "$env_file" ] || continue
